...
const TeamsPlot = () => {
    ...
    const [ndata, setNData] = useState<number[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const sign = await axios.get('http://10.2.0.50:4015/teams_sign');
                const rawsign = sign.data;

                const meeting = await axios.get('http://10.2.0.50:4015/teams_meeting');
                const rawmeeting = meeting.data;

                const slow = await axios.get('http://10.2.0.50:4015/teams_slow');
                const rawslow = slow.data;

                const notchat = await axios.get('http://10.2.0.50:4015/teams_notchat');
                ...
                const slowcounts: number[] = [];

                for (let i = startDate.getTime(); i <= endDate.getTime(); i += interval) {
                    const count = rawslow.filter((item: any) => {
                        const itemDate = moment(item.timestamp).toDate();
                        return itemDate.getTime() >= i && itemDate.getTime() < i + interval;
                    }).length;

                    slowcounts.push(count);
                }
